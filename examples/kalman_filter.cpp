...
//  auto kf = kalman_filter::KalmanFilter(A, B, C, Q, R, P);
  kalman_filter::KalmanFilter kf(A, B, C, Q, R, P);
  ...
  kalman_filter::VectorXt<double> x0(n);
  double t = 0;
  ...
  std::cout << "t = " << t << "\tx_hat[0]: " << kf.state().transpose()
            << std::endl;
  ...
  for (int i = 0; i < measurements.size(); i++) {
    ...
    std::cout << "t = " << t << "\ty[" << i << "] = " << y.transpose()
              ...
              << std::endl;
//    std::cout << kf.stateCovar() << std::endl;
  }
